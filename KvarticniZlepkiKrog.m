...
% w2 mora bit ve? kot -1/3.
w2 = 1/3;
% x1 mora ležati znotraj -((3w2 + 1)/2)^0.5 in ((3w2 + 1)/2)^0.5
% z lambda povemo razmerje kjer leži x1 znotraj tega intervala.
lambda = 0.8;
x1 = -((3*w2 + 1)/2)^0.5 + 2*lambda*((3*w2 + 1)/2)^0.5;
% Izra?unana alfa iz izpeljave
alfa = (3*w2/2 - x1^2 + 1/2)^0.5;

% Kontrolne to?ke, ki jih dolo?a ena izmed rešitev ena?b.
B = [1, 0, 1;
    x1,alfa, x1;
    -(3*w2 - 4*x1^2 + 2)/3, 4/3 * x1*alfa, w2;
    -x1, -alfa, -x1;
    1,0,1];
